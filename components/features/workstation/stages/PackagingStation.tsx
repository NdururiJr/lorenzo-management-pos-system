 ...
 *
 * Interface for packaging staff to package and finalize orders.
 * Packaging is handled per-order, not in batches.
 * This is the final stage before orders are ready for collection/delivery.
 *
 * @module components/features/workstation/stages/PackagingStation
 */
...
import { Loader2, Package, Inbox, CheckCircle, ChevronDown, ChevronUp, Tag } from 'lucide-react';
...
export function PackagingStation() {
  ...
  // Fetch orders at packaging stage
  const { data: packagingOrders = [], refetch, isLoading } = useQuery({
  ...
  const handleCompleteGarment = async (order: Order, garmentId: string) => {
    ...
    try {
      ...
      // Mark garment as complete for packaging stage
      ...
      if (allComplete) {
        ...
        toast.success('Garment packaged successfully');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Packaging Station:</strong> Package each garment carefully with appropriate
          covers and tags. This is the final stage before orders are ready for customer
          collection or delivery. When all garments in an order are packaged, the order will
          automatically move to READY status.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Packaging Queue ({packagingOrders.length})
          </CardTitle>
          <CardDescription>Orders ready for final packaging</CardDescription>
        </CardHeader>
        <CardContent>
          {packagingOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              <Inbox className="w-12 h-12 mb-4 text-gray-300" />
              <p className="text-sm">No orders at packaging stage</p>
            </div>
          ) : (
            <div className="space-y-4">
              {packagingOrders.map((order) => {
                const completedCount = order.garments.filter(
                  (g) => g.stageHandlers?.packaging && g.stageHandlers.packaging.length > 0
                ).length;
                  ...
                  <div key={order.orderId} className="border rounded-lg">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          {completedCount}/{totalCount} Packaged
                        </Badge>
                      ...
                      <div className="p-4 pt-0 space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {order.garments.map((garment) => {
                            const isComplete =
                              garment.stageHandlers?.packaging &&
                              garment.stageHandlers.packaging.length > 0;
                              ...
                              >
                                ...
                                {!isComplete && (
                                  <div className="mb-3 p-2 bg-indigo-50 rounded border border-indigo-200">
                                    ...
                                    <div className="text-xs text-indigo-800 space-y-1">
                                      <div>✓ Garment properly pressed</div>
                                      <div>✓ Protective cover applied</div>
                                      <div>✓ Customer tag attached</div>
                                      <div>✓ Order ID label attached</div>
                                    </div>
                                  </div>
                                )}

                                {/* Completion Info */}
                                {isComplete && garment.stageHandlers?.packaging?.[0] && (
                                  <div className="text-xs text-gray-600 mt-3 pt-3 border-t border-green-200">
                                    <div>
                                      Packaged by: {garment.stageHandlers.packaging[0].name}
                                    </div>
                                    <div>
                                      {format(
                                        garment.stageHandlers.packaging[0].completedAt.toDate(),
                                        'MMM d, h:mm a'
                                      )}
                                    </div>
                                  </div>
                                ...
                                {!isComplete && (
                                  <Button
                                    ...
                                    className="w-full mt-3 bg-indigo-600 text-white hover:bg-indigo-700"
                                    size="sm"
                                  >
                                    {isProcessing ? (
                                      <>
